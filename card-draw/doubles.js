...
const tournament = "rip115doubles";
...
const discordWebhookUrl =
  ...
  "Z1fZ" +
  "RJRz8jFYv" +
  "1IwqNIfdH5xIY-w_Ud"; // so bots don't scrape it
...
$(document).ready(() => {
  ...
  function getGoodSongIndices(diff_lo, diff_hi, mode) {
    ...
    for (let i = 0; i < songs.length; i += 1) {
      let diff_in_bound =
        diff_lo <= songs[i].difficulty && songs[i].difficulty <= diff_hi;
  ...
  function randomize(numRequested, diff_lo, diff_hi, mode) {
    ...
    for (
      let i = 0;
      i < numRequested && goodSongIndicesArray.length > 0;
      i += 1
    ) {
      ...
      goodSongIndicesArray[x] =
        goodSongIndicesArray[goodSongIndicesArray.length - 1];
  ...
  function render(cardArray) {
    ...
    for (let i = 0; i < cardArray.length; i += 1) {
            ...
            <div class="card_regular">
                <div class="banner_image">
                    <div class="info_name">
                        ...
                        <div class="text_english">${songObject.english}</div>
      ...
      `);
      if (typeof songObject.english === "undefined") {
        img.find(".text_english").remove();
      }
