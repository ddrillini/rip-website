...
// NOTE THIS IS RIPPED OFF OF THE NORMAL CARD draw
// VARIABLES WILL SAY 'SONGS' EVEN THOUGH I MEAN 'GAMES'
...
$(document).ready(() => {
  ...
  var jqXHR = $.getJSON(`res/${tournament}/mrgsrt.json`, (data) => {
    songs = data;
  })
  ...
  const statuses = ['card_regular', 'card_vetoed'];
  ...
  function render(cardArray) {
    ...
    for (let i = 0; i < cardArray.length; i += 1) {
                    ...
                    <div class="card_body">
                        <div class="info_bar">
                            <div class="info_title">
                                <div class="text_title">${songObject}</div>
                            </div>
                            <div class="info_difficulty">
                                <div class="text_difficulty_marker">Lv</div>
                                <div class="text_difficulty">??</div>
                            </div>
                        </div>
                        <div class="banner_image"></div>
                        <div class="content_bar">
                            <div class="info_content_title">
                                <div class="text_content_title">${songObject}</div>
                            </div>
                        </div>
                    </div>
                </div>
      ...
      `);
      img.find('.banner_image').css('background-image', `url("res/${tournament}/banners/__generic.png")`);
      img.status = 0;
  ...
  function draw(number) {
    ...
    const randomNumberArray = randomize(number, 0, 0, 0); // shit that doesn't matter here
